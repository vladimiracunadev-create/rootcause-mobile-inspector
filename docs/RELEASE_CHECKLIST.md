...
# Ej.: version: 0.1.1+2 — semver Y build number incrementados respecto al release anterior
```
...
```bash
gh release view v0.1.1 --json body --jq .body | grep -i "modo de firma"
```

- [ ] Hash SHA-256 verificado en local — debe coincidir carácter a carácter con `SHA256SUMS.txt`:

```powershell
gh release download v0.1.1 -p '*universal.apk' -p 'SHA256SUMS.txt' -D "$env:TEMP\rc-verify" --clobber
